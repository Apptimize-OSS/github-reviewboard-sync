...
def _create_pull_request_helper(repo, remote_repo, branch, base):
    ...
    try:
        ...
        if 'errors' in exc.data and len(exc.data['errors']) == 1 and \
            ...
            _LOGGER.warning('A pull request already exists for "{0}".  Continuing.'.format(branch))
        else:
            raise exc
...
def _instantiate_github(username):
    ...
    while True:
        ...
        try:
            ...
            _LOGGER.error('The password was not valid for "{0}"'.format(username))
...
def create_pull_request(path, branch, base, username, remote):
    ...
    _create_pull_request_helper(repo, remote_repo, branch, base)
